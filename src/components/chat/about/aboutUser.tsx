...
export default function AboutUser({friend, setIsFriendInfoOpen}: {friend: friend, setIsFriendInfoOpen: Function}) {
  return(
    <div className={styles.aboutUser} onClick={(e) => {e.stopPropagation();setIsFriendInfoOpen(false)}}>
      <div className={styles.userInfo}>
        <div className={styles.pfpblock}>
          <Image src={friend.avatar} width={120} height={120} alt="avatar"/>
        </div>
        <div className={styles.infoblock}>
          <div>
            ...
            <span className={styles.row}><Icon.Calendar color="#fff" width="26px" height="20px"/> {calculateDate(friend.createdAt.toString(), 'full')}</span>
          </div>
        </div>
      </div>
    </div>
